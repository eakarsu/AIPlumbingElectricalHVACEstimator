...
// Photo/video of job site + job description; AI generates itemized quote (materials, labor hours, disposal); auto-calculates markup; generates professional proposal
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / cf-ai-job-estimator',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_ai_job_estimator`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-ai-job-estimator feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'AI job estimator'.
Context: Photo/video of job site + job description; AI generates itemized quote (materials, labor hours, disposal); auto-calculates markup; generates professional proposal
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-ai-job-estimator/run error:', err);
