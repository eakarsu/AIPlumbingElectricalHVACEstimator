...
// Route material/design through code checker; flag violations by jurisdiction; suggest compliant alternatives
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / cf-code-compliance-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_code_compliance_automation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-code-compliance-automation feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Code compliance automation'.
Context: Route material/design through code checker; flag violations by jurisdiction; suggest compliant alternatives
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-code-compliance-automation/run error:', err);
