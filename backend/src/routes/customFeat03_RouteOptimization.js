...
// Multi-job scheduling; AI optimizes technician routes (minimize travel time, consolidate nearby jobs); predicts arrival times
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / cf-route-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_route_optimization`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-route-optimization feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Route optimization'.
Context: Multi-job scheduling; AI optimizes technician routes (minimize travel time, consolidate nearby jobs); predicts arrival times
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-route-optimization/run error:', err);
