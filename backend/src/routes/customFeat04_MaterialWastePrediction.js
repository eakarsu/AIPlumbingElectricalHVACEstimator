...
// Batch material orders; AI predicts waste (plumbing job = 15% scrap); suggests purchasing efficiency or recycle options
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / cf-material-waste-prediction',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_material_waste_prediction`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-material-waste-prediction feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Material waste prediction'.
Context: Batch material orders; AI predicts waste (plumbing job = 15% scrap); suggests purchasing efficiency or recycle options
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-material-waste-prediction/run error:', err);
