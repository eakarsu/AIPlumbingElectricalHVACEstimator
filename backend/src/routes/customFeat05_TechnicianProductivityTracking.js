...
// Track quote-to-completion time by technician; AI identifies bottlenecks (material delays, rework), suggests improvements
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / cf-technician-productivity-tracking',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_technician_productivity_tracking`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-technician-productivity-tracking feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Technician productivity tracking'.
Context: Track quote-to-completion time by technician; AI identifies bottlenecks (material delays, rework), suggests improvements
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-technician-productivity-tracking/run error:', err);
