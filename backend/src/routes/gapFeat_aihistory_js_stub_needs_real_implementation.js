...
// Gap Feature: \'aiHistory.js\' stub needs real implementation
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / gap-aihistory-js-stub-needs-real-implementation',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: '\'aiHistory.js\' stub needs real implementation'.\nProject: AIPlumbingElectricalHVACEstimator.\nContext: aiHistory.js stub needs real implementation\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['aihistory-js-stub-needs-real-implementation', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'aihistory-js-stub-needs-real-implementation', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
