...
// Gap Feature: Code compliance without \'/code
// Code compliance without /code-compliance-check-ai (NLP rule interpretation)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / gap-code-compliance-without-code',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Code compliance without \'/code'.\nProject: AIPlumbingElectricalHVACEstimator.\nContext: Code compliance without /code-compliance-check-ai (NLP rule interpretation)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['code-compliance-without-code', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'code-compliance-without-code', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
