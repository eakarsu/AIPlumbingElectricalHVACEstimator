...
// Gap Feature: JobQuotes without \'/quote
// JobQuotes without /quote-estimate-ai (auto-generate material list from job description)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / gap-jobquotes-without-quote',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'JobQuotes without \'/quote'.\nProject: AIPlumbingElectricalHVACEstimator.\nContext: JobQuotes without /quote-estimate-ai (auto-generate material list from job description)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['jobquotes-without-quote', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'jobquotes-without-quote', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
