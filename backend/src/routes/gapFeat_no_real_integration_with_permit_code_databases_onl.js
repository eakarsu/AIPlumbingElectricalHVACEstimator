...
// Gap Feature: No real integration with permit/code databases (only generic integrations module)
// No real integration with permit/code databases (only generic integrations module)
// Project: AIPlumbingElectricalHVACEstimator
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / gap-no-real-integration-with-permit-code-databases-onl',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No real integration with permit/code databases (only generic integrations module)'.\nProject: AIPlumbingElectricalHVACEstimator.\nContext: No real integration with permit/code databases (only generic integrations module)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-real-integration-with-permit-code-databases-onl', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-real-integration-with-permit-code-databases-onl', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-real-integration-with-permit-code-databases-onl']
    );
