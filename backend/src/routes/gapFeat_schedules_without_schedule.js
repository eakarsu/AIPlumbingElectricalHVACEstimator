...
// Gap Feature: Schedules without \'/schedule
// Schedules without /schedule-optimize (route optimization for multi-job days)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPlumbingElectricalHVACEstimator / gap-schedules-without-schedule',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Schedules without \'/schedule'.\nProject: AIPlumbingElectricalHVACEstimator.\nContext: Schedules without /schedule-optimize (route optimization for multi-job days)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['schedules-without-schedule', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'schedules-without-schedule', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
