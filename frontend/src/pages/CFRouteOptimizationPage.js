...
// Custom Feature: Route optimization
// Route optimization: Multi-job scheduling; AI optimizes technician routes (minimize travel time, consolidate nearby jobs); predicts arrival times
import React, { useState } from 'react';

export default function CFRouteOptimizationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-route-optimization/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Route optimization</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Route optimization: Multi-job scheduling; AI optimizes technician routes (minimize travel time, consolidate nearby jobs); predicts arrival times</p>
