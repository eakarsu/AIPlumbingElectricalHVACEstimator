...
// Gap (Non-AI): Limited photo documentation (before/after)
// Limited photo documentation (before/after)
import React, { useState } from 'react';

export default function GapLimitedPhotoDocumentationBeforeAfterPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-photo-documentation-before-after/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited photo documentation (before/after)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited photo documentation (before/after)</p>
