...
// Gap (Non-AI): No accounting system integration (QuickBooks, FreshBooks)
// No accounting system integration (QuickBooks, FreshBooks)
import React, { useState } from 'react';

export default function GapNoAccountingSystemIntegrationQuickbooksFreshbPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-accounting-system-integration-quickbooks-freshb/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No accounting system integration (QuickBooks, FreshBooks)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No accounting system integration (QuickBooks, FreshBooks)</p>
