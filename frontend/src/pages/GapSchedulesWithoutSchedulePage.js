...
// Gap (AI): Schedules without '/schedule
// Schedules without /schedule-optimize (route optimization for multi-job days)
import React, { useState } from 'react';

export default function GapSchedulesWithoutSchedulePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-schedules-without-schedule/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Schedules without '/schedule</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Schedules without /schedule-optimize (route optimization for multi-job days)</p>
